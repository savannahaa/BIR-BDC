...
int main()
{
    ...
    if (!loadKeysAndGenerateValues(keys, vals, keyPath, valPath)) {
        cerr << "[p3] loadKeysAndGenerateValues failed" << endl;
        return 1;
    }

    cout << "[p3] Loaded " << keys.size()
    ...
    if (!encodeOKVS_dispatch(bits, keys, vals, D, pp, 0)) {
        cerr << "[p3] encodeOKVS_dispatch failed" << endl;
        return 1;
    }

    cout << "[p3] D encoded: " << D.rows() << " x " << D.cols() << endl;
    ...
    if (::inet_pton(AF_INET, serverIp, &addr.sin_addr) <= 0) {
        cerr << "[p3] invalid server ip: " << serverIp << endl;
        ::close(sock);
        return 1;
    }
    cout << "[p3] Connecting to " << serverIp << ":" << port << " ..." << endl;
    if (::connect(sock, reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) < 0) {
        perror("[p3] connect");
        ::close(sock);
        return 1;
    }
    cout << "[p3] Connected." << endl;
    ...
    if (!sendAll(sock, &rows_n, sizeof(rows_n)) ||
        ...
        cerr << "[p3] send rows/cols failed" << endl;
        ::close(sock);
        return 1;
    }
    ...
    if (dataBytes > 0) {
        if (!sendAll(sock, D.data(), dataBytes)) {
            cerr << "[p3] send D.data() failed" << endl;
            ::close(sock);
            return 1;
        }
    }

    cout << "[p3] Sent D to server, bytes = " << (16 + dataBytes) << endl;
