...
int main(int argc, char** argv) {
    try {
        ...
        double leader_sent_bytes = clients * 16.0 + pivot_share.table.size() * sizeof(Block);
        double client_sent_bytes = client_okvs.empty() ? 0.0 : client_okvs.back().table.size() * sizeof(Block);
        double pivot_psi_sent_bytes = pivot_values.size() * sizeof(Block);
        double leader_psi_sent_bytes = n * sizeof(Block);
        double leader_sent_mb = leader_sent_bytes / (1024.0 * 1024.0);
        double client_sent_mb = client_sent_bytes / (1024.0 * 1024.0);
        double pivot_psi_sent_mb = pivot_psi_sent_bytes / (1024.0 * 1024.0);
        double leader_psi_sent_mb = leader_psi_sent_bytes / (1024.0 * 1024.0);
        double total_sent_mb = leader_sent_mb + clients * client_sent_mb + pivot_psi_sent_mb + leader_psi_sent_mb;
        double total_sent_bytes = leader_sent_bytes + clients * client_sent_bytes + pivot_psi_sent_bytes + leader_psi_sent_bytes;
        double total_seconds = seconds(t0, t_psi);
        auto modeled_total_seconds = [&](double mbps) {
            return total_seconds + (total_sent_bytes * 8.0) / (mbps * 1000.0 * 1000.0);
        };
        double total_1gbps = modeled_total_seconds(1000.0);
        double total_100mbps = modeled_total_seconds(100.0);
        double total_10mbps = modeled_total_seconds(10.0);
        ...
        std::cout << "  modeled_total_seconds_1Gbps: " << total_1gbps << "\n";
        std::cout << "  modeled_total_seconds_100Mbps: " << total_100mbps << "\n";
        std::cout << "  modeled_total_seconds_10Mbps: " << total_10mbps << "\n";
        std::cout << "  peak_RSS_MB: " << rss_mb() << "\n\n";

        std::cout << "csv_header: parties_n,set_size_m,intersection,compute_seconds,total_communication_MB,total_seconds_1Gbps,total_seconds_100Mbps,total_seconds_10Mbps,peak_RSS_MB\n";
        std::cout << "csv: " << args.parties << "," << n << "," << args.intersection << ","
                  << total_seconds << "," << total_sent_mb << ","
                  << total_1gbps << "," << total_100mbps << "," << total_10mbps << ","
                  << rss_mb() << "\n";
